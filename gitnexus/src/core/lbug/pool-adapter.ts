...
async function doInitLbug(repoId: string, dbPath: string): Promise<void> {
  ...
  // Symmetric VECTOR LOAD: each pooled connection is a fresh session that
  // does not inherit the VECTOR extension from the analyze process. Without
  // this, QUERY_VECTOR_INDEX fails and semantic search silently falls back
  // to exact-scan (which OOMs under concurrent queries). load-only mirrors
  // the Fts policy above.
